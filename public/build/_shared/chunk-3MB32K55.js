...
function createRouter(init) {
  ...
  async function handleAction(request, location, submission, matches, isFogOfWar, opts) {
    ...
    if (isRedirectResult(result)) {
      ...
      if (opts && opts.replace != null) {
        replace5 = opts.replace;
      } else {
        let location2 = normalizeRedirectLocation(result.response.headers.get("Location"), new URL(request.url), basename);
        replace5 = location2 === state.location.pathname + state.location.search;
      }
  ...
  async function handleLoaders(request, location, matches, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace5, initialHydration, flushSync, pendingActionResult) {
    ...
    let redirect8 = findRedirect(loaderResults);
    if (redirect8) {
      await startRedirectNavigation(request, redirect8.result, true, {
        replace: replace5
      });
      return {
        shortCircuited: true
    ...
    redirect8 = findRedirect(fetcherResults);
    if (redirect8) {
      fetchRedirectIds.add(redirect8.key);
      await startRedirectNavigation(request, redirect8.result, true, {
        replace: replace5
      });
      return {
  ...
  async function handleFetcherAction(key, routeId, path, match, requestMatches, isFogOfWar, flushSync, preventScrollReset, submission) {
    ...
    let redirect8 = findRedirect(loaderResults);
    if (redirect8) {
      return startRedirectNavigation(revalidationRequest, redirect8.result, false, {
        preventScrollReset
      });
    }
    redirect8 = findRedirect(fetcherResults);
  ...
  async function startRedirectNavigation(request, redirect8, isNavigation, _temp2) {
    let {
      submission,
      fetcherSubmission,
      preventScrollReset,
    ...
    if (redirect8.response.headers.has("X-Remix-Revalidate")) {
      isRevalidationRequired = true;
    }
    let location = redirect8.response.headers.get("Location");
    ...
    if (isBrowser2) {
      ...
      if (redirect8.response.headers.has("X-Remix-Reload-Document")) {
    ...
    pendingNavigationController = null;
    let redirectHistoryAction = replace5 === true || redirect8.response.headers.has("X-Remix-Replace") ? Action.Replace : Action.Push;
    let {
      formMethod,
      formAction,
      formEncType
    ...
    if (redirectPreserveMethodStatusCodes.has(redirect8.response.status) && activeSubmission && isMutationMethod(activeSubmission.formMethod)) {
...
async function convertDataStrategyResultToDataResult(dataStrategyResult) {
  ...
  if (isResponse(result)) {
    ...
    if (type === ResultType.error) {
      return {
        type: ResultType.error,
        error: new ErrorResponseImpl(result.status, result.statusText, data4),
...
function getLoadingFetcher(submission, data4) {
  if (submission) {
    let fetcher = {
      state: "loading",
      formMethod: submission.formMethod,
...
var init_router = __esm({
  "node_modules/react-router/node_modules/@remix-run/router/dist/router.js"() {
    ...
    json = function json2(data4, init) {
      if (init === void 0) {
        init = {};
      }
      ...
      return new Response(JSON.stringify(data4), _extends({}, responseInit, {
        headers
      }));
    };
    ...
    DeferredData = class {
      constructor(data4, responseInit) {
        this.pendingKeysSet = /* @__PURE__ */ new Set();
        this.subscribers = /* @__PURE__ */ new Set();
        this.deferredKeys = [];
        invariant(data4 && typeof data4 === "object" && !Array.isArray(data4), "defer() only accepts plain objects");
        let reject;
        ...
        this.data = Object.entries(data4).reduce((acc, _ref2) => {
          let [key, value] = _ref2;
      ...
      trackPromise(key, value) {
        ...
        let promise = Promise.race([value, this.abortPromise]).then((data4) => this.onSettle(promise, key, void 0, data4), (error) => this.onSettle(promise, key, error));
        promise.catch(() => {
        });
      ...
      onSettle(promise, key, error, data4) {
    ...
    defer = function defer2(data4, init) {
      if (init === void 0) {
        init = {};
      }
      let responseInit = typeof init === "number" ? {
      ...
      return new DeferredData(data4, responseInit);
    };
    ...
    ErrorResponseImpl = class {
      constructor(status, statusText, data4, internal) {
        if (internal === void 0) {
          internal = false;
        }
        this.status = status;
...
function Navigate(_ref4) {
  ...
  React.useEffect(() => navigate(JSON.parse(jsonPath), {
    ...
    state,
    relative
  }), [navigate, jsonPath, relative, replace5, state]);
  return null;
}
...
function ResolveAwait(_ref8) {
  ...
  let data4 = useAsyncValue();
  let toRender = typeof children === "function" ? children(data4) : children;
    ...
    AwaitErrorBoundary = class extends React.Component {
      ...
      render() {
        ...
        if (!(resolve instanceof Promise)) {
          ...
          promise = resolve.then((data4) => Object.defineProperty(resolve, "_data", {
            get: () => data4
...
__export(router_exports, {
  ...
  isDataWithResponseInit: () => isDataWithResponseInit2,
...
function data(data4, init) {
  return new DataWithResponseInit(data4, typeof init === "number" ? {
    status: init
  } : init);
}
...
function createRouter2(init) {
  ...
  async function handleLoaders(request, location, matches, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace5, initialHydration, flushSync, pendingActionResult) {
    ...
    revalidatingFetchers.forEach((rf) => fetchControllers.delete(rf.key));
    let redirect8 = findRedirect2(loaderResults);
    if (redirect8) {
      await startRedirectNavigation(request, redirect8.result, true, {
        replace: replace5
      });
    ...
    redirect8 = findRedirect2(fetcherResults);
    if (redirect8) {
      fetchRedirectIds.add(redirect8.key);
      await startRedirectNavigation(request, redirect8.result, true, {
        replace: replace5
  ...
  async function startRedirectNavigation(request, redirect8, isNavigation, _temp2) {
    ...
    pendingNavigationController = null;
    let redirectHistoryAction = replace5 === true || redirect8.response.headers.has("X-Remix-Replace") ? Action2.Replace : Action2.Push;
    let {
      formMethod,
      formAction,
      formEncType
    ...
    let activeSubmission = submission || fetcherSubmission;
    if (redirectPreserveMethodStatusCodes2.has(redirect8.response.status) && activeSubmission && isMutationMethod2(activeSubmission.formMethod)) {
      await startNavigation(redirectHistoryAction, redirectLocation, {
...
function createStaticHandler(routes, opts) {
  ...
  async function queryRoute(request, _temp4) {
    ...
    if (result.loaderData) {
      ...
      let data4 = Object.values(result.loaderData)[0];
      if ((_result$activeDeferre = result.activeDeferreds) != null && _result$activeDeferre[match.route.id]) {
        data4[UNSAFE_DEFERRED_SYMBOL2] = result.activeDeferreds[match.route.id];
      }
      return data4;
    }
    return void 0;
  ...
  async function queryImpl(request, location, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch) {
    ...
    try {
      if (isMutationMethod2(request.method.toLowerCase())) {
        let result2 = await submit(request, matches, routeMatch || getTargetMatch2(matches, location), requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch != null);
        return result2;
      }
      ...
      if (isDataStrategyResult(e) && isResponse2(e.result)) {
  ...
  async function submit(request, matches, actionMatch, requestContext, dataStrategy, skipLoaderErrorBubbling, isRouteRequest) {
    ...
    if (isErrorResult2(result)) {
      let boundaryMatch = skipLoaderErrorBubbling ? actionMatch : findNearestBoundary2(matches, actionMatch.route.id);
  ...
  async function loadRouteData(request, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch, pendingActionResult) {
    ...
    let requestMatches = routeMatch ? [routeMatch] : pendingActionResult && isErrorResult2(pendingActionResult[1]) ? getLoaderMatchesUntilBoundary2(matches, pendingActionResult[0]) : matches;
    ...
    let activeDeferreds = /* @__PURE__ */ new Map();
    let context = processRouteLoaderData2(matches, results, pendingActionResult, activeDeferreds, skipLoaderErrorBubbling);
  ...
  async function callDataStrategy(type, request, matchesToLoad, matches, isRouteRequest, requestContext, dataStrategy) {
    let results = await callDataStrategyImpl2(dataStrategy || defaultDataStrategy2, type, null, request, matchesToLoad, matches, null, manifest, mapRouteProperties2, requestContext);
    let dataResults = {};
    await Promise.all(matches.map(async (match) => {
      ...
      if (isRedirectDataStrategyResultResult2(result)) {
        let response = result.result;
        throw normalizeRelativeRoutingRedirectResponse2(response, request, match.route.id, matches, basename, future.v7_relativeSplatPath);
      }
      if (isResponse2(result.result) && isRouteRequest) {
        throw result;
      }
      dataResults[match.route.id] = await convertDataStrategyResultToDataResult2(result);
    }));
    return dataResults;
  }
  return {
...
async function convertDataStrategyResultToDataResult2(dataStrategyResult) {
  ...
  if (isResponse2(result)) {
    ...
    if (type === ResultType2.error) {
      return {
        type: ResultType2.error,
        error: new ErrorResponseImpl2(result.status, result.statusText, data4),
        statusCode: result.status,
        headers: result.headers
      };
    }
...
function isDataStrategyResult(result) {
  return result != null && typeof result === "object" && "type" in result && "result" in result && (result.type === ResultType2.data || result.type === ResultType2.error);
}
...
function getLoadingFetcher2(submission, data4) {
  if (submission) {
    let fetcher = {
      state: "loading",
      formMethod: submission.formMethod,
...
function getDoneFetcher2(data4) {
  let fetcher = {
    state: "idle",
    formMethod: void 0,
    formAction: void 0,
...
var init_router2 = __esm({
  "node_modules/react-router-dom/node_modules/@remix-run/router/dist/router.js"() {
    ...
    json3 = function json4(data4, init) {
      if (init === void 0) {
        init = {};
      }
      let responseInit = typeof init === "number" ? {
      ...
      return new Response(JSON.stringify(data4), _extends3({}, responseInit, {
        headers
      }));
    };
    DataWithResponseInit = class {
      constructor(data4, init) {
    ...
    DeferredData2 = class {
      constructor(data4, responseInit) {
        ...
        this.subscribers = /* @__PURE__ */ new Set();
        this.deferredKeys = [];
        invariant2(data4 && typeof data4 === "object" && !Array.isArray(data4), "defer() only accepts plain objects");
        let reject;
        this.abortPromise = new Promise((_, r) => reject = r);
        this.controller = new AbortController();
    ...
    defer3 = function defer4(data4, init) {
      if (init === void 0) {
        init = {};
      }
      let responseInit = typeof init === "number" ? {
      ...
      return new DeferredData2(data4, responseInit);
    };
...
function useLinkClickHandler(to, _temp) {
  ...
  return React2.useCallback((event) => {
    if (shouldProcessLinkClick(event, target)) {
      ...
      let replace5 = replaceProp !== void 0 ? replaceProp : createPath(location) === createPath(path);
      navigate(to, {
        replace: replace5,
        state,
        preventScrollReset,
...
function useFetcher(_temp3) {
  ...
  let data4 = fetcherData.get(fetcherKey);
  let fetcherWithComponents = React2.useMemo(() => _extends4({
    ...
    data: data4
  }), [FetcherForm, submit, load, fetcher, data4]);
...
var React2, ReactDOM, defaultMethod, defaultEncType, _formDataSupportsSubmitter, supportedFormEncTypes, _excluded, _excluded2, _excluded3, REACT_ROUTER_VERSION, ViewTransitionContext, FetchersContext, START_TRANSITION2, startTransitionImpl2, FLUSH_SYNC, flushSyncImpl, USE_ID, useIdImpl, Deferred, MemoizedDataRoutes, isBrowser, ABSOLUTE_URL_REGEX3, Link, NavLink, Form, DataRouterHook2, DataRouterStateHook2, fetcherId, getUniqueFetcherId, SCROLL_RESTORATION_STORAGE_KEY, savedScrollPositions;
  ...
  "node_modules/react-router-dom/server.js"(exports) {
    ...
    function StaticRouterProvider2({
      ...
      if (hydrate2 !== false) {
        ...
        let json8 = htmlEscape(JSON.stringify(JSON.stringify(data4)));
...
function matchRoutes3(routes, locationArg, basename) {
  ...
  return matchRoutesImpl3(routes, locationArg, basename, false);
}
function matchRoutesImpl3(routes, locationArg, basename, allowPartial) {
  ...
  for (let i = 0; matches == null && i < branches.length; ++i) {
    ...
    matches = matchRouteBranch3(branches[i], decoded, allowPartial);
  }
  return matches;
}
...
function matchRouteBranch3(branch, pathname, allowPartial) {
  if (allowPartial === void 0) {
    allowPartial = false;
  }
  let {
...
function resolveTo3(toArg, routePathnames, locationPathname, isPathRelative) {
  ...
  if (typeof toArg === "string") {
    ...
    to = _extends5({}, toArg);
    invariant3(!to.pathname || !to.pathname.includes("?"), getInvalidPathError3("?", "pathname", "search", to));
    invariant3(!to.pathname || !to.pathname.includes("#"), getInvalidPathError3("#", "pathname", "hash", to));
    invariant3(!to.search || !to.search.includes("#"), getInvalidPathError3("#", "search", "hash", to));
  }
  let isEmptyPath = toArg === "" || to.pathname === "";
  let toPathname = isEmptyPath ? "/" : to.pathname;
  let from;
...
var json5 = function json6(data4, init) {
  if (init === void 0) {
    init = {};
  }
  let responseInit = typeof init === "number" ? {
  ...
  return new Response(JSON.stringify(data4), _extends5({}, responseInit, {
    headers
  }));
};
var DataWithResponseInit2 = class {
  constructor(data4, init) {
    this.type = "DataWithResponseInit";
    this.data = data4;
    this.init = init || null;
...
function data2(data4, init) {
  return new DataWithResponseInit2(data4, typeof init === "number" ? {
    status: init
  } : init);
}
...
var DeferredData3 = class {
  constructor(data4, responseInit) {
    ...
    this.subscribers = /* @__PURE__ */ new Set();
    this.deferredKeys = [];
    invariant3(data4 && typeof data4 === "object" && !Array.isArray(data4), "defer() only accepts plain objects");
    let reject;
    this.abortPromise = new Promise((_, r) => reject = r);
    this.controller = new AbortController();
...
var defer5 = function defer6(data4, init) {
  if (init === void 0) {
    init = {};
  }
  let responseInit = typeof init === "number" ? {
  ...
  return new DeferredData3(data4, responseInit);
};
...
function createRouter3(init) {
  ...
  let inFlightDataRoutes;
  let basename = init.basename || "/";
  let dataStrategyImpl = init.dataStrategy || defaultDataStrategy3;
  let patchRoutesOnNavigationImpl = init.patchRoutesOnNavigation;
  ...
  if (!initialMatches) {
    ...
    if (errors) {
      let idx = initialMatches.findIndex((m) => errors[m.route.id] !== void 0);
      initialized = initialMatches.slice(0, idx + 1).every((m) => !shouldLoadRouteOnHydration3(m.route, loaderData, errors));
    } else {
      initialized = initialMatches.every((m) => !shouldLoadRouteOnHydration3(m.route, loaderData, errors));
    }
  } else {
    initialized = init.hydrationData != null;
  }
  ...
  async function startNavigation(historyAction, location, opts) {
    ...
    let matches = opts != null && opts.initialHydration && state.matches && state.matches.length > 0 && !initialMatchesIsFOW ? (
      // `matchRoutes()` has already been called if we're in here via `router.initialize()`
      state.matches
    ) : matchRoutes3(routesToUse, location, basename);
    let flushSync = (opts && opts.flushSync) === true;
    if (matches && state.initialized && !isRevalidationRequired && isHashChangeOnly3(state.location, location) && !(opts && opts.submission && isMutationMethod3(opts.submission.formMethod))) {
      completeNavigation(location, {
        matches
      }, {
        flushSync
    ...
    let pendingActionResult;
    if (opts && opts.pendingError) {
      pendingActionResult = [findNearestBoundary3(matches).route.id, {
      ...
      if (actionResult.pendingActionResult) {
        let [routeId, result] = actionResult.pendingActionResult;
        if (isErrorResult3(result) && isRouteErrorResponse3(result.error) && result.error.status === 404) {
          pendingNavigationController = null;
          completeNavigation(location, {
            matches: actionResult.matches,
            loaderData: {},
      ...
      flushSync = false;
      fogOfWar.active = false;
      request = createClientSideRequest3(init.history, request.url, request.signal);
    }
    let {
      shortCircuited,
      matches: updatedMatches,
    ...
    completeNavigation(location, _extends5({
      matches: updatedMatches || matches
    }, getActionDataForCommit3(pendingActionResult), {
      loaderData,
      errors
    }));
  }
  async function handleAction(request, location, submission, matches, isFogOfWar, opts) {
    ...
    if (isFogOfWar) {
      ...
      if (discoverResult.type === "aborted") {
        ...
        let boundaryId = findNearestBoundary3(discoverResult.partialMatches).route.id;
        return {
          matches: discoverResult.partialMatches,
          pendingActionResult: [boundaryId, {
            type: ResultType3.error,
    ...
    if (isRedirectResult3(result)) {
      ...
      if (opts && opts.replace != null) {
        ...
        let location2 = normalizeRedirectLocation3(result.response.headers.get("Location"), new URL(request.url), basename);
        replace5 = location2 === state.location.pathname + state.location.search;
      }
      await startRedirectNavigation(request, result, true, {
        submission,
  ...
  async function handleLoaders(request, location, matches, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace5, initialHydration, flushSync, pendingActionResult) {
    ...
    let routesToUse = inFlightDataRoutes || dataRoutes;
    let [matchesToLoad, revalidatingFetchers] = getMatchesToLoad3(init.history, state, matches, activeSubmission, location, future.v7_partialHydration && initialHydration === true, future.v7_skipActionErrorRevalidation, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionResult);
    cancelActiveDeferreds((routeId) => !(matches && matches.some((m) => m.route.id === routeId)) || matchesToLoad && matchesToLoad.some((m) => m.route.id === routeId));
    pendingNavigationLoadId = ++incrementingLoadId;
    if (matchesToLoad.length === 0 && revalidatingFetchers.length === 0) {
      let updatedFetchers2 = markFetchRedirectsDone();
      completeNavigation(location, _extends5({
        ...
        loaderData: {},
        // Commit pending error if we're short circuiting
        errors: pendingActionResult && isErrorResult3(pendingActionResult[1]) ? {
          [pendingActionResult[0]]: pendingActionResult[1].error
        } : null
      }, getActionDataForCommit3(pendingActionResult), updatedFetchers2 ? {
        fetchers: new Map(state.fetchers)
      } : {}), {
        flushSync
      });
    ...
    revalidatingFetchers.forEach((rf) => fetchControllers.delete(rf.key));
    let redirect8 = findRedirect3(loaderResults);
    if (redirect8) {
      await startRedirectNavigation(request, redirect8.result, true, {
        replace: replace5
      });
    ...
    redirect8 = findRedirect3(fetcherResults);
    if (redirect8) {
      fetchRedirectIds.add(redirect8.key);
      await startRedirectNavigation(request, redirect8.result, true, {
        replace: replace5
    ...
    let {
      loaderData,
      errors
    } = processLoaderData3(state, matches, loaderResults, pendingActionResult, revalidatingFetchers, fetcherResults, activeDeferreds);
    activeDeferreds.forEach((deferredData, routeId) => {
      deferredData.subscribe((aborted) => {
        if (aborted || deferredData.done) {
          activeDeferreds.delete(routeId);
    ...
    if (future.v7_partialHydration && initialHydration && state.errors) {
      errors = _extends5({}, state.errors, errors);
    }
    let updatedFetchers = markFetchRedirectsDone();
    let didAbortFetchLoads = abortStaleFetchLoads(pendingNavigationLoadId);
    let shouldUpdateFetchers = updatedFetchers || didAbortFetchLoads || revalidatingFetchers.length > 0;
  ...
  function getUpdatedActionData(pendingActionResult) {
    if (pendingActionResult && !isErrorResult3(pendingActionResult[1])) {
      return {
        [pendingActionResult[0]]: pendingActionResult[1].data
      };
    } else if (state.actionData) {
  ...
  async function handleFetcherAction(key, routeId, path, match, requestMatches, isFogOfWar, flushSync, preventScrollReset, submission) {
    ...
    if (isFogOfWar) {
      ...
      if (discoverResult.type === "aborted") {
        ...
        requestMatches = discoverResult.matches;
        match = getTargetMatch3(requestMatches, path);
        if (detectAndHandle405Error(match)) {
          return;
        }
      }
    ...
    state.fetchers.set(key, loadFetcher);
    let [matchesToLoad, revalidatingFetchers] = getMatchesToLoad3(init.history, state, matches, submission, nextLocation, false, future.v7_skipActionErrorRevalidation, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, [match.route.id, actionResult]);
    revalidatingFetchers.filter((rf) => rf.key !== key).forEach((rf) => {
      let staleKey = rf.key;
      let existingFetcher2 = state.fetchers.get(staleKey);
    ...
    let {
      loaderData,
      errors
    } = processLoaderData3(state, matches, loaderResults, void 0, revalidatingFetchers, fetcherResults, activeDeferreds);
    if (state.fetchers.has(key)) {
  ...
  async function handleFetcherLoader(key, routeId, path, match, matches, isFogOfWar, flushSync, preventScrollReset, submission) {
    ...
    if (isFogOfWar) {
      ...
      if (discoverResult.type === "aborted") {
        ...
        matches = discoverResult.matches;
        match = getTargetMatch3(matches, path);
      }
    }
    fetchControllers.set(key, abortController);
    let originatingLoadId = incrementingLoadId;
    ...
    invariant3(!isDeferredResult3(result), "Unhandled fetcher deferred data");
  ...
  async function startRedirectNavigation(request, redirect8, isNavigation, _temp2) {
    ...
    let location = redirect8.response.headers.get("Location");
    invariant3(location, "Expected a Location header on the redirect Response");
    location = normalizeRedirectLocation3(location, new URL(request.url), basename);
    let redirectLocation = createLocation3(state.location, location, {
      _isRedirect: true
    });
    if (isBrowser2) {
      let isDocumentReload = false;
      if (redirect8.response.headers.has("X-Remix-Reload-Document")) {
        isDocumentReload = true;
      } else if (ABSOLUTE_URL_REGEX4.test(location)) {
        const url = init.history.createURL(location);
        isDocumentReload = // Hard reload if it's an absolute URL to a new origin
        url.origin !== routerWindow.location.origin || // Hard reload if it's an absolute URL that does not match our basename
    ...
    pendingNavigationController = null;
    let redirectHistoryAction = replace5 === true || redirect8.response.headers.has("X-Remix-Replace") ? Action3.Replace : Action3.Push;
    let {
      formMethod,
      formAction,
      formEncType
    ...
    if (!submission && !fetcherSubmission && formMethod && formAction && formEncType) {
      submission = getSubmissionFromNavigation3(state.navigation);
    }
    let activeSubmission = submission || fetcherSubmission;
    if (redirectPreserveMethodStatusCodes3.has(redirect8.response.status) && activeSubmission && isMutationMethod3(activeSubmission.formMethod)) {
      await startNavigation(redirectHistoryAction, redirectLocation, {
  ...
  async function callDataStrategy(type, state2, request, matchesToLoad, matches, fetcherKey) {
    ...
    let dataResults = {};
    try {
      results = await callDataStrategyImpl3(dataStrategyImpl, type, state2, request, matchesToLoad, matches, fetcherKey, manifest, mapRouteProperties2);
    } catch (e) {
      matchesToLoad.forEach((m) => {
        dataResults[m.route.id] = {
          type: ResultType3.error,
    ...
    for (let [routeId, result] of Object.entries(results)) {
      if (isRedirectDataStrategyResultResult3(result)) {
        let response = result.result;
        dataResults[routeId] = {
          type: ResultType3.redirect,
          response: normalizeRelativeRoutingRedirectResponse3(response, request, routeId, matches, basename, future.v7_relativeSplatPath)
        };
      } else {
        dataResults[routeId] = await convertDataStrategyResultToDataResult3(result);
      }
    }
    return dataResults;
  }
  async function callLoadersAndMaybeResolveData(state2, matches, matchesToLoad, fetchersToLoad, request) {
    ...
    let fetcherResultsPromise = Promise.all(fetchersToLoad.map(async (f) => {
      if (f.matches && f.match && f.controller) {
        let results = await callDataStrategy("loader", state2, createClientSideRequest3(init.history, f.path, f.controller.signal), [f.match], f.matches, f.key);
        let result = results[f.match.route.id];
        return {
          [f.key]: result
        };
    ...
    let loaderResults = await loaderResultsPromise;
    let fetcherResults = (await fetcherResultsPromise).reduce((acc, r) => Object.assign(acc, r), {});
    await Promise.all([resolveNavigationDeferredResults3(matches, loaderResults, request.signal, currentMatches, state2.loaderData), resolveFetcherDeferredResults3(matches, fetcherResults, fetchersToLoad)]);
    return {
      loaderResults,
      fetcherResults
    };
  ...
  function checkFogOfWar(matches, routesToUse, pathname) {
    if (patchRoutesOnNavigationImpl) {
      if (!matches) {
        let fogMatches = matchRoutesImpl3(routesToUse, pathname, basename, true);
        return {
          active: true,
          matches: fogMatches || []
        };
        ...
        if (Object.keys(matches[0].params).length > 0) {
          let partialMatches = matchRoutesImpl3(routesToUse, pathname, basename, true);
          return {
            active: true,
            matches: partialMatches
          };
  ...
  async function discoverRoutes(matches, pathname, signal, fetcherKey) {
    ...
    while (true) {
      ...
      try {
        await patchRoutesOnNavigationImpl({
          ...
          patch: (routeId, children) => {
            if (signal.aborted)
              return;
            patchRoutesImpl3(routeId, children, routesToUse, localManifest, mapRouteProperties2);
          }
        });
      } catch (e) {
        return {
      ...
      let newMatches = matchRoutes3(routesToUse, pathname, basename);
      if (newMatches) {
        return {
          type: "success",
          matches: newMatches
      ...
      let newPartialMatches = matchRoutesImpl3(routesToUse, pathname, basename, true);
      if (!newPartialMatches || partialMatches.length === newPartialMatches.length && partialMatches.every((m, i) => m.route.id === newPartialMatches[i].route.id)) {
        return {
          type: "success",
          matches: null
  ...
  function patchRoutes(routeId, children) {
    let isNonHMR = inFlightDataRoutes == null;
    let routesToUse = inFlightDataRoutes || dataRoutes;
    patchRoutesImpl3(routeId, children, routesToUse, manifest, mapRouteProperties2);
    if (isNonHMR) {
      dataRoutes = [...dataRoutes];
      updateState({});
    }
...
function normalizeTo3(location, matches, basename, prependBasename, to, v7_relativeSplatPath, fromRouteId, relative) {
  ...
  if ((to == null || to === "" || to === ".") && activeRouteMatch) {
    let nakedIndex = hasNakedIndexQuery3(path.search);
    if (activeRouteMatch.route.index && !nakedIndex) {
      path.search = path.search ? path.search.replace(/^\?/, "?index&") : "?index";
    } else if (!activeRouteMatch.route.index && nakedIndex) {
      let params = new URLSearchParams(path.search);
...
function getLoaderMatchesUntilBoundary3(matches, boundaryId, includeBoundary) {
  if (includeBoundary === void 0) {
    includeBoundary = false;
  }
  let index = matches.findIndex((m) => m.route.id === boundaryId);
...
function getMatchesToLoad3(history, state, matches, submission, location, initialHydration, skipActionErrorRevalidation, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionResult) {
  let actionResult = pendingActionResult ? isErrorResult3(pendingActionResult[1]) ? pendingActionResult[1].error : pendingActionResult[1].data : void 0;
  let currentUrl = history.createURL(state.location);
  let nextUrl = history.createURL(location);
  let boundaryMatches = matches;
  if (initialHydration && state.errors) {
    boundaryMatches = getLoaderMatchesUntilBoundary3(matches, Object.keys(state.errors)[0], true);
  } else if (pendingActionResult && isErrorResult3(pendingActionResult[1])) {
    boundaryMatches = getLoaderMatchesUntilBoundary3(matches, pendingActionResult[0]);
  }
  let actionStatus = pendingActionResult ? pendingActionResult[1].statusCode : void 0;
  let shouldSkipRevalidation = skipActionErrorRevalidation && actionStatus && actionStatus >= 400;
  let navigationMatches = boundaryMatches.filter((match, index) => {
    ...
    if (initialHydration) {
      return shouldLoadRouteOnHydration3(route, state.loaderData, state.errors);
    }
    ...
    let currentRouteMatch = state.matches[index];
    let nextRouteMatch = match;
    return shouldRevalidateLoader3(match, _extends5({
      currentUrl,
      currentParams: currentRouteMatch.params,
      nextUrl,
      nextParams: nextRouteMatch.params
  ...
  fetchLoadMatches.forEach((f, key) => {
    ...
    if (fetchRedirectIds.has(key)) {
      ...
      shouldRevalidate = shouldRevalidateLoader3(fetcherMatch, _extends5({
        currentUrl,
        currentParams: state.matches[state.matches.length - 1].params,
        nextUrl,
        nextParams: matches[matches.length - 1].params
...
function shouldLoadRouteOnHydration3(route, loaderData, errors) {
  if (route.lazy) {
    return true;
  }
  if (!route.loader) {
...
function patchRoutesImpl3(routeId, children, routesToUse, manifest, mapRouteProperties2) {
  var _childrenToPatch;
  let childrenToPatch;
  if (routeId) {
    let route = manifest[routeId];
    invariant3(route, "No route found to patch children into: routeId = " + routeId);
    if (!route.children) {
      route.children = [];
    }
    childrenToPatch = route.children;
  ...
  let uniqueChildren = children.filter((newRoute) => !childrenToPatch.some((existingRoute) => isSameRoute3(newRoute, existingRoute)));
  let newRoutes = convertRoutesToDataRoutes3(uniqueChildren, mapRouteProperties2, [routeId || "_", "patch", String(((_childrenToPatch = childrenToPatch) == null ? void 0 : _childrenToPatch.length) || "0")], manifest);
  childrenToPatch.push(...newRoutes);
}
function isSameRoute3(newRoute, existingRoute) {
  if ("id" in newRoute && "id" in existingRoute && newRoute.id === existingRoute.id) {
    return true;
  }
  if (!(newRoute.index === existingRoute.index && newRoute.path === existingRoute.path && newRoute.caseSensitive === existingRoute.caseSensitive)) {
  ...
  return newRoute.children.every((aChild, i) => {
    var _existingRoute$childr;
    return (_existingRoute$childr = existingRoute.children) == null ? void 0 : _existingRoute$childr.some((bChild) => isSameRoute3(aChild, bChild));
  });
}
...
async function defaultDataStrategy3(_ref4) {
  let {
    matches
  } = _ref4;
  let matchesToLoad = matches.filter((m) => m.shouldLoad);
...
async function callDataStrategyImpl3(dataStrategyImpl, type, state, request, matchesToLoad, matches, fetcherKey, manifest, mapRouteProperties2, requestContext) {
  let loadRouteDefinitionsPromises = matches.map((m) => m.route.lazy ? loadLazyRouteModule3(m.route, mapRouteProperties2, manifest) : void 0);
  let dsMatches = matches.map((match, i) => {
    let loadRoutePromise = loadRouteDefinitionsPromises[i];
    let shouldLoad = matchesToLoad.some((m) => m.route.id === match.route.id);
    let resolve = async (handlerOverride) => {
      ...
      return shouldLoad ? callLoaderOrAction3(type, request, match, loadRoutePromise, handlerOverride, requestContext) : Promise.resolve({
        type: ResultType3.data,
        result: void 0
      });
    };
    return _extends5({}, match, {
      shouldLoad,
      resolve
    });
  });
...
async function callLoaderOrAction3(type, request, match, loadRoutePromise, handlerOverride, staticContext) {
  let result;
  let onReject;
  let runHandler = (handler) => {
    let reject;
  ...
  try {
    ...
    invariant3(result.result !== void 0, "You defined " + (type === "action" ? "an action" : "a loader") + " for route " + ('"' + match.route.id + "\" but didn't return anything from your `" + type + "` ") + "function. Please return a value or `null`.");
  } catch (e) {
    return {
      type: ResultType3.error,
      result: e
...
async function convertDataStrategyResultToDataResult3(dataStrategyResult) {
  let {
    result,
    type
  } = dataStrategyResult;
  if (isResponse3(result)) {
    ...
    if (type === ResultType3.error) {
      return {
        type: ResultType3.error,
        error: new ErrorResponseImpl3(result.status, result.statusText, data4),
        statusCode: result.status,
        headers: result.headers
      };
    }
  ...
  if (type === ResultType3.error) {
    if (isDataWithResponseInit3(result)) {
      var _result$init3, _result$init4;
      if (result.data instanceof Error) {
        var _result$init, _result$init2;
        return {
      ...
      return {
        type: ResultType3.error,
        error: new ErrorResponseImpl3(((_result$init3 = result.init) == null ? void 0 : _result$init3.status) || 500, void 0, result.data),
...
function normalizeRelativeRoutingRedirectResponse3(response, request, routeId, matches, basename, v7_relativeSplatPath) {
  let location = response.headers.get("Location");
  ...
  if (!ABSOLUTE_URL_REGEX4.test(location)) {
    let trimmedMatches = matches.slice(0, matches.findIndex((m) => m.route.id === routeId) + 1);
    location = normalizeTo3(new URL(request.url), trimmedMatches, basename, true, location, v7_relativeSplatPath);
    response.headers.set("Location", location);
  }
  return response;
}
function normalizeRedirectLocation3(location, currentUrl, basename) {
  if (ABSOLUTE_URL_REGEX4.test(location)) {
    let normalizedLocation = location;
    let url = normalizedLocation.startsWith("//") ? new URL(currentUrl.protocol + normalizedLocation) : new URL(normalizedLocation);
...
function processRouteLoaderData3(matches, results, pendingActionResult, activeDeferreds, skipLoaderErrorBubbling) {
  let loaderData = {};
  let errors = null;
  let statusCode;
  let foundError = false;
  let loaderHeaders = {};
  let pendingError = pendingActionResult && isErrorResult3(pendingActionResult[1]) ? pendingActionResult[1].error : void 0;
  matches.forEach((match) => {
    if (!(match.route.id in results)) {
      return;
    }
    let id = match.route.id;
    let result = results[id];
    invariant3(!isRedirectResult3(result), "Cannot handle redirect results in processLoaderData");
    if (isErrorResult3(result)) {
      let error = result.error;
      if (pendingError !== void 0) {
        error = pendingError;
...
function processLoaderData3(state, matches, results, pendingActionResult, revalidatingFetchers, fetcherResults, activeDeferreds) {
  let {
    loaderData,
    errors
  } = processRouteLoaderData3(
    matches,
    results,
    pendingActionResult,
    activeDeferreds,
  ...
  revalidatingFetchers.forEach((rf) => {
    ...
    let result = fetcherResults[key];
    invariant3(result, "Did not find corresponding fetcher result");
    if (controller && controller.signal.aborted) {
      return;
...
function getActionDataForCommit3(pendingActionResult) {
  if (!pendingActionResult) {
    return {};
  }
  return isErrorResult3(pendingActionResult[1]) ? {
    // Clear out prior actionData on errors
    actionData: {}
  } : {
    actionData: {
...
function stripHashFromPath3(path) {
  ...
  return createPath3(_extends5({}, parsedPath, {
    hash: ""
  }));
}
...
function isRedirectDataStrategyResultResult3(result) {
  return isResponse3(result.result) && redirectStatusCodes3.has(result.result.status);
}
...
function isDataWithResponseInit3(value) {
  return typeof value === "object" && value != null && "type" in value && "data" in value && "init" in value && value.type === "DataWithResponseInit";
}
...
async function resolveNavigationDeferredResults3(matches, results, signal, currentMatches, currentLoaderData) {
  let entries = Object.entries(results);
  for (let index = 0; index < entries.length; index++) {
    let [routeId, result] = entries[index];
    let match = matches.find((m) => (m == null ? void 0 : m.route.id) === routeId);
    ...
    if (isDeferredResult3(result) && isRevalidatingLoader) {
      await resolveDeferredData3(result, signal, false).then((result2) => {
        if (result2) {
          results[routeId] = result2;
        }
      });
...
async function resolveFetcherDeferredResults3(matches, results, revalidatingFetchers) {
  for (let index = 0; index < revalidatingFetchers.length; index++) {
    let {
      key,
      routeId,
    ...
    if (isDeferredResult3(result)) {
      invariant3(controller, "Expected an AbortController for revalidating fetcher deferred result");
      await resolveDeferredData3(result, controller.signal, true).then((result2) => {
        if (result2) {
          results[key] = result2;
        }
      });
...
function getLoadingFetcher3(submission, data4) {
  if (submission) {
    let fetcher = {
      state: "loading",
      formMethod: submission.formMethod,
...
function getDoneFetcher3(data4) {
  let fetcher = {
    state: "idle",
    formMethod: void 0,
    formAction: void 0,
...
// node_modules/@remix-run/react/node_modules/@remix-run/server-runtime/dist/esm/responses.js
var json7 = (data4, init = {}) => {
  return json5(data4, init);
};
var defer7 = (data4, init = {}) => {
  return defer5(data4, init);
};
var redirect7 = (url, init = 302) => {
  return redirect5(url, init);
};
var replace4 = (url, init = 302) => {
  return replace3(url, init);
};
var redirectDocument4 = (url, init = 302) => {
  return redirectDocument3(url, init);
};

// node_modules/@remix-run/react/node_modules/turbo-stream/dist/turbo-stream.mjs
var HOLE = -1;
var NAN = -2;
...
// node_modules/@remix-run/react/node_modules/@remix-run/server-runtime/dist/esm/single-fetch.js
var SingleFetchRedirectSymbol = Symbol("SingleFetchRedirect");
function data3(value, init) {
  return data2(value, init);
}
...
async function fetchData(request, routeId, retry = 0) {
  ...
  if (isErrorResponse(response)) {
    ...
    let error = new Error(data4.message);
    error.stack = data4.stack;
    return error;
  }
    ...
    void (async () => {
      try {
        for await (let section of sectionReader) {
          ...
          let data4 = JSON.parse(sectionDataString);
          if (event === "data") {
...
async function singleFetchActionStrategy(request, matches) {
  ...
  invariant4(actionMatch, "No action match found");
  ...
  let result = await actionMatch.resolve(async (handler) => {
    let result2 = await handler(async () => {
      ...
      actionStatus = status;
      return unwrapSingleFetchResult(data4, actionMatch.route.id);
    });
    return result2;
  });
  if (isResponse4(result.result) || isRouteErrorResponse3(result.result)) {
    return {
  ...
  return {
    [actionMatch.route.id]: {
      type: result.type,
      result: data2(result.result, actionStatus)
    }
  };
}
async function singleFetchLoaderNavigationStrategy(manifest, routeModules, router2, request, matches) {
  ...
  let resolvePromise = Promise.all(matches.map(async (m, i) => m.resolve(async (handler) => {
    ...
    try {
      let result = await handler(async () => {
        let data4 = await singleFetchDfd.promise;
        return unwrapSingleFetchResults(data4, m.route.id);
      });
      results[m.route.id] = {
        type: "data",
        result
  ...
  if ((!router2.state.initialized || routesParams.size === 0) && !window.__remixHdrActive) {
    ...
    try {
      ...
      let data4 = await fetchAndDecode(url, init);
      singleFetchDfd.resolve(data4.data);
    } catch (e) {
...
async function singleFetchLoaderFetcherStrategy(request, matches) {
  ...
  invariant4(fetcherMatch, "No fetcher match found");
...
function fetchSingleLoader(handler, url, init, routeId) {
  return handler(async () => {
    ...
    return unwrapSingleFetchResults(data4, routeId);
  });
}
...
async function fetchAndDecode(url, init) {
  ...
  invariant4(res.body, "No response body to decode");
  try {
  ...
  return decode(body, {
    plugins: [(type, ...rest) => {
      ...
      if (type === "ErrorResponse") {
        let [data4, status, statusText] = rest;
        return {
          value: new ErrorResponseImpl3(status, statusText, data4)
        };
      }
...
function unwrapSingleFetchResults(results, routeId) {
  let redirect8 = results[SingleFetchRedirectSymbol];
  if (redirect8) {
    return unwrapSingleFetchResult(redirect8, routeId);
  }
...
function unwrapSingleFetchResult(result, routeId) {
  if ("error" in result) {
    ...
    throw redirect5(result.redirect, {
...
function getRedirect(response) {
  ...
  let replace5 = response.headers.get("X-Remix-Replace");
  if (replace5) {
    headers["X-Remix-Replace"] = replace5;
  }
  return redirect(url, {
    status,
    headers
...
function getPartialManifest(manifest, router2) {
  ...
  paths.forEach((path) => {
    let matches = matchRoutes3(router2.routes, path, router2.basename);
    if (matches) {
...
var NavLink2 = /* @__PURE__ */ React8.forwardRef(({
  ...
  return /* @__PURE__ */ React8.createElement(React8.Fragment, null, /* @__PURE__ */ React8.createElement(NavLink, _extends6({}, props, prefetchHandlers, {
...
var Link2 = /* @__PURE__ */ React8.forwardRef(({
  ...
  let href = useHref(to);
  let [shouldPrefetch, ref, prefetchHandlers] = usePrefetchBehavior(prefetch, props);
  return /* @__PURE__ */ React8.createElement(React8.Fragment, null, /* @__PURE__ */ React8.createElement(Link, _extends6({}, props, prefetchHandlers, {
    ref: mergeRefs(forwardedRef, ref),
    to,
    "data-discover": getDiscoverAttr(discover, isAbsolute, props.reloadDocument)
  })), shouldPrefetch && !isAbsolute ? /* @__PURE__ */ React8.createElement(PrefetchPageLinks, {
...
var Form2 = /* @__PURE__ */ React8.forwardRef(({
  ...
  let isAbsolute = typeof props.action === "string" && ABSOLUTE_URL_REGEX5.test(props.action);
  return /* @__PURE__ */ React8.createElement(Form, _extends6({}, props, {
    ref: forwardedRef,
    "data-discover": getDiscoverAttr(discover, isAbsolute, props.reloadDocument)
  }));
});
...
function Links() {
  ...
  return /* @__PURE__ */ React8.createElement(React8.Fragment, null, criticalCss ? /* @__PURE__ */ React8.createElement("style", {
    ...
    key,
    link
  }) => isPageLinkDescriptor(link) ? /* @__PURE__ */ React8.createElement(PrefetchPageLinks, _extends6({
    key
  }, link)) : /* @__PURE__ */ React8.createElement("link", _extends6({
    key
  }, link))));
}
function PrefetchPageLinks({
  ...
  return /* @__PURE__ */ React8.createElement(PrefetchPageLinksImpl, _extends6({
    page,
    matches
  }, dataLinkProps));
}
...
function PrefetchPageLinksImpl({
  ...
  return /* @__PURE__ */ React8.createElement(React8.Fragment, null, dataHrefs.map((href) => /* @__PURE__ */ React8.createElement("link", _extends6({
    key: href,
    rel: "prefetch",
    as: "fetch",
    href
  }, linkProps))), moduleHrefs.map((href) => /* @__PURE__ */ React8.createElement("link", _extends6({
    key: href,
    rel: "modulepreload",
    href
    ...
    /* @__PURE__ */ React8.createElement("link", _extends6({
      key
    }, link))
  )));
}
function Meta() {
  ...
  return /* @__PURE__ */ React8.createElement(React8.Fragment, null, meta.flat().map((metaProps) => {
    ...
    if ("tagName" in metaProps) {
      ...
      let Comp = tagName;
      return /* @__PURE__ */ React8.createElement(Comp, _extends6({
    ...
    return /* @__PURE__ */ React8.createElement("meta", _extends6({
...
function Scripts(props) {
  ...
  let serializePreresolvedDataImp = (routeId, key, data4) => {
    let serializedData;
    try {
      serializedData = JSON.stringify(data4);
    } catch (error) {
  ...
  let serializeDataImp = (routeId, key, data4) => {
    let serializedData;
    try {
      serializedData = JSON.stringify(data4);
    } catch (error) {
...
import(${JSON.stringify(manifest.entry.module)});`;
    return /* @__PURE__ */ React8.createElement(React8.Fragment, null, /* @__PURE__ */ React8.createElement("script", _extends6({}, props, {
      suppressHydrationWarning: true,
      dangerouslySetInnerHTML: createHtml(contextScript),
      type: void 0
    })), /* @__PURE__ */ React8.createElement("script", _extends6({}, props, {
      suppressHydrationWarning: true,
...
function DeferredHydrationScript({
  ...
  return /* @__PURE__ */ React8.createElement(React8.Suspense, {
    fallback: (
      ...
      typeof document === "undefined" && deferredData && dataKey && routeId ? null : /* @__PURE__ */ React8.createElement("script", _extends6({}, scriptProps, {
        async: true,
        suppressHydrationWarning: true,
        dangerouslySetInnerHTML: {
          __html: " "
    ...
    children: (data4) => {
      return /* @__PURE__ */ React8.createElement("script", _extends6({}, scriptProps, {
        async: true,
        suppressHydrationWarning: true,
        dangerouslySetInnerHTML: {
          __html: serializeData(routeId, dataKey, data4)
        }
      }));
    }
  }) : /* @__PURE__ */ React8.createElement("script", _extends6({}, scriptProps, {
    async: true,
    suppressHydrationWarning: true,
    dangerouslySetInnerHTML: {
      __html: " "
  ...
  if (!router) {
    if (window.__remixContext.future.v3_singleFetch) {
      if (!stateDecodingPromise) {
        ...
        invariant4(stream, "No stream found for single fetch decoding");
...
function ScrollRestoration2({
  ...
  return /* @__PURE__ */ React10.createElement("script", _extends6({}, props, {
    suppressHydrationWarning: true,
    dangerouslySetInnerHTML: {
...
@remix-run/react/dist/esm/index.js:
   ...
   *)
*/
//# sourceMappingURL=/build/_shared/chunk-3MB32K55.js.map
